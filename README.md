...
### Preloader 提速與預設語言（2026-09-22）

進度條原本人為下限 1500ms、上限 4200ms（模組晚載入時樓地板
900／2600ms），跑完後還有 hold 與約 900ms 的 wipe 出場動畫，
整段近 3 秒。`index-CEwmJUQ0.js` 中把 `B=500`、`z=1600`、樓地板
350／1100、最短停留 `K=400`、強制 booted 的 fallback `J=1600`；
出場動畫保留原設計。熱快取實測：進度條 1852ms → 約 850ms，
整段 3051ms → 約 1950ms。

語言判定順位為 URL `?lang=` → localStorage → 瀏覽器語言
（zh→中、en→英）→ 最終 fallback。`index-C5_56iHU.js` 中把最終
fallback 由 `"en"` 改為 `"zh"`——中英文以外的瀏覽器（日、韓等）
現在預設看到中文。中文與英文瀏覽器行為不變。
